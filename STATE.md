...
- **DatabaseHandler split** — 🟡 `DatabaseHandler.ts` is 907 lines after moving skill-point build cache generation to `src/services/SkillPointBuildService.ts` and one-shot maintenance jobs to `src/services/DatabaseMaintenanceService.ts`. Next safe target: rollback/fixup scripts (`fixRaidTransactions`, `rollbackDailyClaim`, `rollbackTransaction`) or the old `migrateData` archive block.
