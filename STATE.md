...
- **P3.4 fat command files** — 🟡 `RaidSubcommands.ts` down to 791 lines after extracting `src/rpg/SeasonalRaids.ts` (seasonal raid boss definitions + `getFixedBosses`). `Dungeon.ts` down to 737 lines after extracting `src/commands/combat/dungeon_config.ts` (modifier table + multiplier helpers + drop table). The remaining `giveRewards` block (~320 lines) is the next high-value extract.
