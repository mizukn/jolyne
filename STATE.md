...
- **P3.1 Functions.ts split** — 🟡 active. Down from 3,071 → 1,696 lines (−45% so far). Sibling modules under `src/utils/` peeled so far: `lookup.ts` (find* helpers), `quest_guards.ts` (is*Quest type guards), `quest_factories.ts` (generate*Quest + push* helpers; lives next to `random.ts` which now also owns `generateRandomId`), `quest_ui.ts` (quest list V2 row helpers + side quest requirement summaries), `rewards.ts` (reward formulas + total XP), `format.ts` (`localeNumber` / `romanize` / `msToString` / `sleep` / string/date/display helpers / Discord timestamps), `embed.ts` (`fixFields` / `splitEmbedIfExceedsLimit` / embed message helpers), `date.ts`, `item_guards.ts`, and `math.ts`. User-domain helpers, prestige, and user-data diffing now live in `src/services/UserService.ts`, with Functions.ts aliases for back-compat. Bulk still inside (~1,700 lines). Next obvious peels per PLAN: `domain/items/inventory.ts`, `domain/items/equipment.ts`, `domain/stats/*`.
