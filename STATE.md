...
- **DatabaseHandler split** — 🟡 `DatabaseHandler.ts` is 1,010 lines after moving skill-point build cache generation to `src/services/SkillPointBuildService.ts`. Next safe target: one-shot maintenance jobs (`fixSettingsToEveryone`, `removeGreenBabies*`, `resetEveryoneCoins`, `removeConsumables`) as free functions that take the handler + client.
