...
export default class RedisHandler {
    ...
    constructor() {
        ...
        this.client = new redis({ db: process.env.DEV_MODE === "true" ? 2 : 1 })
        .on('connect', () => {
            log("Connected.", "redis");
    ...
    async hgetall(key: string): Promise<Record<string, any> > {
        const data = await this.client.hgetall(key) as Record<string, string | number>;
    ...
    async hmset(key: string, jsonData: {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        [key: string]: any;
    }): Promise<"OK"> {
