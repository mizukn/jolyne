...
export default function log(content: string, type = "log"): void {
	...
	switch (type) {
		...
		case "cmd":
			console.log(
				`${clusterID}${getDatePrefix()} ${chalk.blueBright("[CMD]")} ${content}`
			);
			break;
		case "redis":
			console.log(
				`${clusterID}${getDatePrefix()} ${chalk.black.bgRedBright(
					"[REDIS]"
				)} ${content}`
			);
			break;
		...
		case "event":
			console.log(
				`${clusterID}${getDatePrefix()} ${chalk.yellow("[EVENT]")} ${content}`
			);
			break;
		default:
			console.log(
				`${clusterID}${getDatePrefix()} ${chalk.hex("#70926c")(
					"[LOG]"
				)} ${content}`
			);
	}
