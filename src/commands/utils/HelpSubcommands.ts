...
const slashCommand: SlashCommandFile = {
    ...
    execute: async (ctx: CommandInteractionContext): Promise<InteractionResponse | Message | void> => {
        if (ctx.interaction.options.getString("command")) {
            ...
            const embed: APIEmbed = {
                ...
                description: ctx.client.getSlashCommandMention(command.name) + ": " + command.description,
                fields: [],
                color: 0x70926c
            };
            if (command.options) {
                embed.fields.push({
                    name: "Options",
                    value: `> ${ctx.client.getSlashCommandMention(command.name)} \`${command.options.sort((a, b) => {
                        // required first
                        if (a.required && !b.required) return -1;
                        if (!a.required && b.required) return 1;
                        return a.name.localeCompare(b.name);
                    }).map(x => `${x.required ? `<${x.name}>` : `[${x.name}]`}`).join(" ")}\`\n` + command.options.sort((a, b) => {
                        // required first
                        if (a.required && !b.required) return -1;
                        if (!a.required && b.required) return 1;
                        return a.name.localeCompare(b.name);
                    }).map(x => ` - \`${x.name}:\` ${x.description} ${x.required ? " (required)" : ""}`).join("\n")
                });
            }
            embed.fields.push({
                name: "Category",
