...
export default class DatabaseHandler {
    ...
    async reloadRPGUsers(): Promise<void> {
        ...
        for (const row of result.rows) {
            ...
            await this.setJSONData(`${process.env.REDIS_PREFIX}:${userData.id}`, userData);
        }
    }
    ...
    async setJSONData(key: string, data: LocalRedisJSON): Promise<string> {
        if (key.startsWith(`${process.env.REDIS_PREFIX}:`)) {
            ...
            const query = `INSERT INTO "RPGUsers" (${columns.join(", ")}) VALUES (${values
                .map((v, i) => `$${i + 1}`)
                .join(", ")}) ON CONFLICT (id) DO UPDATE SET (${columns.join(", ")}) = (${values
                .map((v, i) => `$${i + 1}`)
                .join(", ")})`;
    ...
    async createUserData(user: string | User, overwrite?: boolean): Promise<RPGUserDataJSON> {
        ...
        if (!overwrite) {
            let data = await this.getJSONData(`${process.env.REDIS_PREFIX}:${user.id}`);
    ...
    async getCooldownCache(base: string, target?: string): Promise<string | null> {
        ...
        return await this.redis.get(`tempCache_${base}:${target}`).then((r) => r || null);
    }
